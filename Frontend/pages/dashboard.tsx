  ...
  return <>
    <div className={`${loading? 'pointer-events-none':''} flex flex-wrap h-screen bg-gray-100 ${theme === 'dark' ? 'bg-zinc-900' : 'bg-gray-100 overflow-hidden'}`}>
      ...
      <div className='w-full h-full bg-transparent lg:px-4'>
        {
          logout ?
            <div onClick={()=>logoutSubmit} className={`transition ease-in-out absolute  h-14 w-60 bg-red-300 p-2 ${theme === 'dark' ? 'bg-black shadow-red-500/50 shadow-lg' : ''} rounded-lg right-2`}>
        ...
        <div className='w-full h-full mx-4 lg:p-8 bg-green-100 '>
          <button onClick={() => { console.log("hey") }} className='w-2/6 lg:w-1/6 h-8 lg:h-6 lg:h-12 border-2  border-black float-right rounded-lg lg:px-8 lg:py-2 px-4 lg:mx-2'>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 float-left">
