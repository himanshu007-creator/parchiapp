...
const FileOptions = ({ children, show,file ,Tok}:Shortcutoptions) => {
    ...
    const fetchUrl = 'https://parchiapp-backend.vercel.appfile/delete/'+fileName+'?q='+Tok
    const detete = ()=>{
        fetch(fetchUrl,{ 
            method: 'get', 
            headers: new Headers({
    ...
    return (
        <div className="group">
            <div className={`${showEnabled ? 'visible' : 'invisible'} group-hover:visible w-30 h-12 relative top-10 float-right -left-2 text-black z-10 pt-4 flex gap-6 top-2`}>
